    ...
    public class ApiClient
    {
        ...
        public async Task<bool> PingAsync(string api)
        {
            try
            {
                ...
                _logger.LogInformation("[PING] Testing connection to: {Url}", fullUrl);
                
                var response = await _httpClient.GetAsync(api);
                
                _logger.LogInformation("[PING] Response: {StatusCode}, Success: {Success}", response.StatusCode, response.IsSuccessStatusCode);
